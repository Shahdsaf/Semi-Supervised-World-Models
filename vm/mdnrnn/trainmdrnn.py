...
def set_logger(logger):
    ...
    filename = os.path.join(logdir, "mdnrnn_train.log")
...
logger.info("Loading VAE at epoch {} "
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    for i, data in enumerate(loader):
        ...
        logger.info("loss={loss:10.6f} bce={bce:10.6f} "
                             "gmm={gmm:10.6f} mse={mse:10.6f}".format(
                                 loss=cum_loss / (i + 1), bce=cum_bce / (i + 1),
                                 gmm=cum_gmm / LSIZE / (i + 1), mse=cum_mse / (i + 1)))
        pbar.update(BSIZE)
...
for e in range(epochs):
    ...
    if earlystopping.stop:
        logger.info("End of Training because of early stopping at epoch {}".format(e))
        break
