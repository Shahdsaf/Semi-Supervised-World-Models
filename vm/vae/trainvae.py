...
def set_logger(logger):
    ...
    filename = os.path.join(logdir, "vae_train.log")
...
if not args.noreload and exists(reload_file):
    ...
    logger.info("Reloading model at epoch {}"
...
for epoch in range(1, args.epochs + 1):
    ...
    if not args.nosamples:
        with torch.no_grad():
            ...
            save_image(ch1,
                       join(vae_dir, 'samples/sample_' + str(epoch) + '_' + str(0) + '.png'))
            save_image(ch2,
                       join(vae_dir, 'samples/sample_' + str(epoch) + '_' + str(1) + '.png'))
            save_image(ch3,
                       join(vae_dir, 'samples/sample_' + str(epoch) + '_' + str(2) + '.png'))
            

    if earlystopping.stop:
        logger.info("End of Training because of early stopping at epoch {}".format(epoch))
        break
